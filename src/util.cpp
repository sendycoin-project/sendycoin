...
const char * const BITCOIN_CONF_FILENAME = "sendycoin.conf";
const char * const BITCOIN_PID_FILENAME = "sendycoin.pid";
...
#else
    const char* pszModule = "sendycoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Sendycoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Sendycoin";
#else
    // Unix
    return pathRet / ".sendycoin";
#endif
#endif
}
